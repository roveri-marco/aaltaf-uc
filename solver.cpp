 ...
 */
 ...
 namespace aalta
 {
   ...
   void Solver::add_clauses_for(aalta_formula *f)
   {
     ...
     assert(f->oper() != aalta_formula::ZYesterday);
     if (clauses_added(f))
       return;
     int id, x_id;
     switch (f->oper())
   ...
   Transition* Solver::get_transition()
   {
     ...
     for (std::vector<int>::iterator it = assign.begin();
	  it != assign.end(); it ++) {
       if ((*it) == 0)
	 continue;
       if (verbose_)
   ...
   {
     // if there is a conjuct A in f such that (A, X A) is not found
     // in X_map_, then discard blocking f
     if (block_X_discard_able(ands))
       return;
     std::vector<int> v;
   ...
   bool Solver::block_X_discard_able(const af_prt_set& ands)
   {
     for (af_prt_set::const_iterator it = ands.begin(); it != ands.end(); it ++) {
